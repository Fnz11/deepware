...
@section('content')
    <section class="container">
        <div class="d-flex align-items-center mb-4">
            ...
            <h2 class="mb-0">FORMULIR ASSESSMENT KEPADA BAWAHAN</h2>
        </div>
            ...
            <div class="card-body">
                <form>
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="nama" value="{{ $atasan->name }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="nip" value="{{ $atasan->nip }}" readonly>
                        </div>
                    </div>
                
                    <div class="row mb-2">
                        <label for="nip_bawahan" class="col-sm-3 col-form-label">NIP Bawahan</label>
                        <div class="col-sm-9">
                            <form method="GET" action="{{ route('assessment.bawahan') }}">
                                <select name="nip_bawahan" class="form-select" onchange="this.form.submit()">
                                    <option selected disabled>Pilih NIP Bawahan</option>
                                    @foreach ($bawahanList as $bawahan)
                                        <option value="{{ $bawahan->nip }}" {{ request('nip_bawahan') == $bawahan->nip ? 'selected' : '' }}>
                                            {{ $bawahan->nip }} - {{ $bawahan->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </form>
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="jabatan" value="{{ $atasan->userPivot?->jabatan?->jabatan }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="unit_kerja" value="{{ $atasan->userPivot?->unitKerja?->unit_kerja }}" readonly>
                        </div>
                    </div>
                </form>
        ...
        <form method="POST" action="{{ route('assessment-atasan.simpan') }}">
        @csrf
        <input type="hidden" name="nip_bawahan" value="{{ request('nip_bawahan') }}">
            ...
            <div class="card mb-4">
                @foreach ($pertanyaanEssay as $index => $item)
                    <div class="mb-2 p-3">
                        ...
                        <textarea class="form-control" name="jawaban_essay[{{ $item->id }}]" id="essay_{{ $index }}" rows="3"></textarea>
                    </div>
                @endforeach
